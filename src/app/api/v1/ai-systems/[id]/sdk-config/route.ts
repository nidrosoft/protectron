...
// GET /api/v1/ai-systems/[id]/sdk-config - Get SDK configuration
  ...
  try {
    ...
    if (!config) {
      return NextResponse.json({
        data: {
          agent_id: id,
          agent_name: aiSystem.name,
          has_config: false,
          sdk_connected: aiSystem.sdk_connected,
          sdk_last_event_at: aiSystem.sdk_last_event_at,
          sdk_events_total: aiSystem.sdk_events_total,
        },
      });
    }
...
// POST /api/v1/ai-systems/[id]/sdk-config - Generate/regenerate API key
export async function POST(
  ...
  try {
    ...
    if (existingConfig) {
      ...
      if (error) {
        return NextResponse.json({ error: "Failed to update SDK config" }, { status: 500 });
      }
      config = data;
    } else {
      // Create new config
      ...
      if (error) {
        return NextResponse.json({ error: "Failed to create SDK config" }, { status: 500 });
      }
      config = data;
    }
    ...
    await supabase.from("activity_log").insert({
      ...
      action_description: `${existingConfig ? "Regenerated" : "Generated"} API key for AI system: ${aiSystem.name}`,
    });
...
// PATCH /api/v1/ai-systems/[id]/sdk-config - Update SDK settings
